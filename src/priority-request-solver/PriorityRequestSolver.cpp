...
{
    readConfigFile();
    getPriorityWeightsAndMethod();
    emergencyVehicle = static_cast<int>(MsgEnum::vehicleType::special);
    transit = static_cast<int>(MsgEnum::vehicleType::bus);
    truck = static_cast<int>(MsgEnum::vehicleType::axleCnt4);
    connectedPassengerVehicle = static_cast<int>(MsgEnum::vehicleType::car);
    nonConnectedVehicle = static_cast<int>(MsgEnum::vehicleType::carOther);
}
...
int PriorityRequestSolver::getMessageType(string jsonString)
{
    ...
    if (parsingSuccessful == true)
    {
        ...
        else if ((jsonObject["MsgType"]).asString() == "ArrivalTableData")
            messageType = static_cast<int>(msgType::arrivalTableData);

        else
        {
            displayConsoleData("Message type is unknown");
            displayConsoleData(jsonString);
...
string PriorityRequestSolver::getScheduleforTCI()
{
    ...
    if (getOptimizationMethod() == "MILP" && !priorityRequestList.empty())
    {
        GLPKSolver();
...
void PriorityRequestSolver::getCurrentSignalStatus(string jsonString)
{
    ...
    if ((transitOrTruckRequestStatus || emergencyVehicleStatus) && getOptimizationMethod() == "MILP")
    {
    ...
    else if ((!transitOrTruckRequestStatus && signalCoordinationRequestStatus) && getOptimizationMethod() == "MILP")
    {
    ...
    if (getOptimizationMethod() == "DP")
    {
        ...
        phaseCallList = trafficConrtollerStatusManager.getPhaseCallList();
    }
}
...
*/
void PriorityRequestSolver::getPriorityWeightsAndMethod()
{
    double currentTime = getPosixTimestamp();
    Json::Value jsonObject;
    ...
    {
        displayConsoleData("Set Optimization Method");
        loggingData("Set Optimization Method");
    }
...
// string PriorityRequestSolver::getTimePhaseDiagramMessageStringForDP()
// {
//     return timePhaseDiagramJsonStringForDP;
// }
...
void PriorityRequestSolver::loggingOptimizationData()
{
    ...
    if (logging && getOptimizationMethod() == "MILP")
    {
        double timeStamp = getPosixTimestamp();
    ...
    if (logging && getOptimizationMethod() == "DP")
    {
        double timeStamp = getPosixTimestamp();
        logFile << "\n[" << fixed << showpoint << setprecision(4) << timeStamp << "] [" << getVerboseTimestamp() << "] Current optimal solution is following:\n\n";
        ...
        for (size_t i = 0; i < optimalSignalTiming.size(); i++)
            logFile << optimalSignalTiming[i].stageNo << "          " << optimalSignalTiming[i].timeStep << "      "
...
string PriorityRequestSolver::getOptimizationMethod()
{
    return optimizationMethod;
}

void PriorityRequestSolver::setArrivalTable(string jsonString)
{
    int noOfVehicles{};
    ...
    loggingData(jsonString);
    noOfVehicles = jsonObject["ArrivalTable"]["NoOfVehicles"].asInt();

    for (int i = 0; i < noOfVehicles; i++)
    {
        arrivableTable.reset();
        arrivableTable.signalGroup = jsonObject["ArrivalTable"]["SignalGroup"][i].asInt();
        arrivableTable.ETA = jsonObject["ArrivalTable"]["ETA"][i].asDouble();
        arrivableTable.vehicleType = jsonObject["ArrivalTable"]["VehicleType"][i].asInt();

        if (arrivableTable.vehicleType == emergencyVehicle)
            arrivableTable.priorityWeight = emergencyVehicleWeight;

        else if (arrivableTable.vehicleType == transit)
            arrivableTable.priorityWeight = transitWeight;

        else if (arrivableTable.vehicleType == truck)
            arrivableTable.priorityWeight = truckWeight;

        else if (arrivableTable.vehicleType == connectedPassengerVehicle)
            arrivableTable.priorityWeight = connectedPassengerVehicleWeight;

        else if (arrivableTable.vehicleType == nonConnectedVehicle)
            arrivableTable.priorityWeight = nonConnectedVehicleWeight;
