...
ScheduleManager::ScheduleManager(vector<COP::OptimalPlan> optimal_Signal_Timing, vector<TrafficControllerData::TrafficConrtollerStatus> traffic_Controller_Status)
{
    if (!optimal_Signal_Timing.empty())
        optimalSignalTiming = optimal_Signal_Timing;

    if (!traffic_Controller_Status.empty())
...
void ScheduleManager::readOptimalSignalPlan()
{
    ...
    /*
    - The OptimizationResults.txt file has 24 phase duration (3cycles). 
    - The total phase duration values for each phases in ring1 will be stored
    - To store the data for ring1, we have to skip the phases of opposite ring.
    */
    for (size_t i = 0; i < leftCriticalPoints.size(); i++)
    {
        leftCriticalPoints_PhaseDuration_Ring1.push_back(leftCriticalPoints.at(i));
        //In total we have 24 phase information. Index set of Ring1 phases is:{0,1,2,3,8,9,10,11,16,17,18,19}
        if (i == 3)
            i = 7;
        else if (i == 11)
            i = 15;
    ...
    /*
    - The OptimizationResults.txt file has 24 green time (3cycles). 
    - The total green time values for each phases in ring2 will be stored
    - To store the data for ring2, we have to skip the phases of opposite ring.
    */
...
void ScheduleManager::createEventListForDP()
{
    Schedule schedule;
    double phaseDurationInRing1{};
    double phaseDurationInRing2{};

    for (size_t i = 0; i < optimalSignalTiming.size(); i++)
    {
        if (i == 0)
        {
            /* For signal group1 ring1*/
            schedule.reset();
            ...
            completeSchedule.push_back(schedule);
            phaseDurationInRing1 = optimalSignalTiming[i].allocatedGreenTimeSignalGroup1Ring1 + optimalSignalTiming[i].clearanceIntervalSignalGroup1Ring1 + trafficControllerStatus[0].initPhase1;

            /* For signal group1 ring2*/
            schedule.reset();
            ...
            completeSchedule.push_back(schedule);
            phaseDurationInRing2 = optimalSignalTiming[i].allocatedGreenTimeSignalGroup1Ring2 + optimalSignalTiming[i].clearanceIntervalSignalGroup1Ring2 + trafficControllerStatus[0].initPhase2;
        }
        ...
        else
        {
            ...
            schedule.commandPhase = optimalSignalTiming[i].signalGroup1InRing1;
            schedule.commandType = "hold";
            schedule.commandStartTime = phaseDurationInRing1; 
            schedule.commandEndTime = phaseDurationInRing1 + optimalSignalTiming[i].allocatedGreenTimeSignalGroup1Ring1 - 0.5;
            completeSchedule.push_back(schedule);
            
            schedule.reset();
            schedule.commandPhase = optimalSignalTiming[i].signalGroup1InRing1;
            schedule.commandType = "forceoff";
            schedule.commandStartTime = phaseDurationInRing1 + optimalSignalTiming[i].allocatedGreenTimeSignalGroup1Ring1;
            schedule.commandEndTime = phaseDurationInRing1 + optimalSignalTiming[i].allocatedGreenTimeSignalGroup1Ring1 + 3.0; 
            completeSchedule.push_back(schedule);
            phaseDurationInRing1 = phaseDurationInRing1 + optimalSignalTiming[i].allocatedGreenTimeSignalGroup1Ring1 + optimalSignalTiming[i].clearanceIntervalSignalGroup1Ring1;

            /* For signal group1 ring2*/
            schedule.reset();
            schedule.commandPhase = optimalSignalTiming[i].signalGroup1InRing2;
            schedule.commandType = "hold";
            schedule.commandStartTime = phaseDurationInRing2;
            schedule.commandEndTime = phaseDurationInRing2 + optimalSignalTiming[i].allocatedGreenTimeSignalGroup1Ring2 - 0.5;
            completeSchedule.push_back(schedule);
            
            schedule.reset();
            schedule.commandPhase = optimalSignalTiming[i].signalGroup1InRing2;
            schedule.commandType = "forceoff";
            schedule.commandStartTime = phaseDurationInRing2 + optimalSignalTiming[i].allocatedGreenTimeSignalGroup1Ring2;
            schedule.commandEndTime = phaseDurationInRing2 + optimalSignalTiming[i].allocatedGreenTimeSignalGroup1Ring2 + 3.0;
            completeSchedule.push_back(schedule);
            phaseDurationInRing2 = phaseDurationInRing2 + optimalSignalTiming[i].allocatedGreenTimeSignalGroup1Ring2 + optimalSignalTiming[i].clearanceIntervalSignalGroup1Ring2;
        
        }

        /* For signal group2 ring1*/
        if (optimalSignalTiming[i].signalGroup2InRing1 > 0)
        {
            schedule.reset();
            schedule.commandPhase = optimalSignalTiming[i].signalGroup2InRing1;
            schedule.commandType = "hold";
            schedule.commandStartTime = phaseDurationInRing1;
            schedule.commandEndTime = phaseDurationInRing1 + optimalSignalTiming[i].allocatedGreenTimeSignalGroup2Ring1 - 0.5;
            completeSchedule.push_back(schedule);
            
            schedule.reset();
            schedule.commandPhase = optimalSignalTiming[i].signalGroup2InRing1;
            schedule.commandType = "forceoff";
            schedule.commandStartTime = phaseDurationInRing1 + optimalSignalTiming[i].allocatedGreenTimeSignalGroup2Ring1;
            schedule.commandEndTime = phaseDurationInRing1 + optimalSignalTiming[i].allocatedGreenTimeSignalGroup2Ring1 + 3.0;
            completeSchedule.push_back(schedule);
            phaseDurationInRing1 = phaseDurationInRing1 + optimalSignalTiming[i].allocatedGreenTimeSignalGroup2Ring1 + optimalSignalTiming[i].clearanceIntervalSignalGroup2Ring1;
        }

        /* For signal group2 ring2*/
        if (optimalSignalTiming[i].signalGroup2InRing2 > 0)
        {
            schedule.reset();
            schedule.commandPhase = optimalSignalTiming[i].signalGroup2InRing2;
            schedule.commandType = "hold";
            schedule.commandStartTime = phaseDurationInRing2;
            schedule.commandEndTime = phaseDurationInRing2 + optimalSignalTiming[i].allocatedGreenTimeSignalGroup2Ring2 - 0.5;
            completeSchedule.push_back(schedule);
            
            schedule.reset();
            schedule.commandPhase = optimalSignalTiming[i].signalGroup2InRing2;
            schedule.commandType = "forceoff";
            schedule.commandStartTime = phaseDurationInRing2 + optimalSignalTiming[i].allocatedGreenTimeSignalGroup2Ring2;
            schedule.commandEndTime = phaseDurationInRing2 + optimalSignalTiming[i].allocatedGreenTimeSignalGroup2Ring2 + 3.0;
            completeSchedule.push_back(schedule);
            phaseDurationInRing2 = phaseDurationInRing2 + optimalSignalTiming[i].allocatedGreenTimeSignalGroup2Ring2 + optimalSignalTiming[i].clearanceIntervalSignalGroup2Ring2;
        }
    }
}
...
{
    bool solutionValidationStatus{false};

    for (unsigned int i = 0; i < completeSchedule.size(); i++)
    {
        if (completeSchedule[i].commandEndTime -completeSchedule[i].commandStartTime > 2.0) //For Force-off point commandEndTime is required to be greater than zero. Thus we chose the checking value as 2.0 (safety factor).
        {
            solutionValidationStatus = true;
            break;
        }
    ...
    return solutionValidationStatus;
}
...
string ScheduleManager::createScheduleJsonString()
{
    ...
    // cout << "Schedule is following: \n" << scheduleJsonString << endl;
